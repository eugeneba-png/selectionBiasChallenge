...
4. **Step 4**: Create block letter "S" matching image dimensions ✅
5. **Step 5**: Create masked image by applying the letter mask to the stippled image ✅
6. **Final**: Assemble all components into the four-panel meme ✅
...
**Steps you'll write:**
- **`step4_create_block_letter.py`**: Block letter generation ✅
- **`step5_create_masked.py`**: Mask application ✅
- **`create_meme.py`**: Final assembly and visualization ✅
...
# Load and prepare the image (put a JPEG/PNG next to index.qmd, or a demo is used)
img_path = "/Users/eugeneasare/Dropbox/Mac/Downloads/IMG_4514.JPG"
...
4. ✅ **Write Step 4**: `step4_create_block_letter.py` generates the block letter "S"
5. ✅ **Write Step 5**: `step5_create_masked.py` applies the mask
6. ✅ **Write Final Step**: `create_meme.py` assembles the four-panel meme
...
meme_png = mpimg.imread("my_statistics_meme.png")
...
> This meme shows selection bias by comparing the full image (Reality) to a stippled sample (Your Model), then marking a systematic missing-data pattern as an “S” (Selection Bias). The last panel (Estimate) keeps stipples only outside that pattern, so the picture we recover is distorted in the same way estimates are biased when data are missing non-randomly.
